...
등장 배선 (Length Matching): 중앙 허브에서 4개의 벌집 슬롯으로 연결되는 모든 신호선은 반드시 동거리 배선이어야 합니다. 제로 지터(Zero-Jitter) 데이터 전송을 보장하기 위해 배선 길이 오차는 ±0.05mm 이내로 제한됩니다.
...
Stacking Height: 4.0mm (Fixed)

Connector: 0.5mm Pitch Board-to-Board (BTB) 40-Pin Header

3. Advanced Signal Integrity & Connectivity
[English] Connector Specification: Mainboard must use 0.5mm Pitch Female (Receptacle) BTB Connectors.

Impedance Standard: The 16-Lane UHI (Universal Honeycomb Interface) requires a strict 100Ω Differential Impedance. Traces must be routed as differential pairs with consistent spacing.

Stacking Precision: The vertical distance between the mainboard and adapter PCB must be maintained at exactly 4.0mm. Any deviation beyond ±0.1mm will result in mechanical failure or signal degradation.

[한국어] 커넥터 규격: 메인보드에는 반드시 0.5mm 피치 암놈(Receptacle) BTB 커넥터를 실장해야 합니다.
